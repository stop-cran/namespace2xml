...
**Contract bundle `r33+925af40f8fec`.** 34 codes.
...
| `WILDCARD001` | error | once per rule | Invalid, undefined, or mixed capture | `source`, `line`, `column`, `path` |
...
| `WARN003` | warning | once per feature category and output file | Unsupported metadata/comment discarded or normalized | `source`, `destination` |
...
### `WILDCARD001` — Invalid, undefined, or mixed capture
...
### `WARN003` — Unsupported metadata/comment discarded or normalized
...
### `WARN009` — Scheme directive binds to no effective output/path or wildcard output creates no instance
