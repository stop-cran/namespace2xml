...
Naming a parser means naming three things, because the bare name is not a claim anyone can act on or test. The documentation states the **reader configuration** the claim holds under, the **envelope** of dialect options and value shapes it holds within, and it states them for each parser separately. A general-purpose INI reader is usually configurable in precisely the places dialects disagree — whether keys keep their case, which characters separate a key from a value, whether the value text is post-processed — so "interoperable with *P*" is true or false only once those settings are fixed, and an implementation that reports the outcome under its own preferred settings while a reader uses the defaults has published a result neither of them can reproduce. Where a dialect feature has no expression in a parser at all, the envelope excludes it rather than the documentation calling the parser incompatible: an option the specification defines and the parser cannot represent is a limit on the pairing, not a defect in either.

A conformance test covering a named parser establishes that the parser, under the stated configuration, recovers the same keys, the same values, and the same order that the emitted file states — not merely that it accepts the file. Acceptance without agreement is the failure this rule exists to catch, because a reader that silently folds a key, splits one in the wrong place, or rewrites a value reports success and returns a different document.
...
- a verdict on namespace2xml 2.4.0, written on its own line as `- namespace2xml 2.4.0: **<verdict>**` and continuing in prose on that line or the lines that follow, where `<verdict>` is exactly one of `agrees`, `differs`, `fails`, or `nondeterministic`;
...
The harness verifies every verdict by observation rather than recording it, and it observes more than once. A single run cannot tell a stable result from a lucky one: 2.4.0 was measured producing three different observable results for one fixture across ten identical runs, and one sample would have reported whichever arrived first as settled fact. Each verdict is therefore a claim about a set of samples of the same case, and the four verdicts partition those samples by how they stand to the case's expected result. `agrees`, and an absent verdict, require every sample to produce the case's expected output tree and expected exit code. `differs` requires every sample to diverge from them and at least one sample to exit zero, so a correction the baseline turns out to reproduce fails as loudly as a compatibility claim it breaks. `fails` requires every sample to diverge and every sample to exit nonzero. `nondeterministic` records a baseline observed to stand in both relations on different runs, and it is the required verdict whenever that is seen, because neither agreement nor difference is then true of the baseline. An unchecked verdict is a comment, and the compatibility policy of Section 3 would then rest on the recollection of whoever wrote it.

The zero-exit clause of `differs` is what makes these classes disjoint rather than nested. Without it every case satisfying `fails` would satisfy `differs` as well, an author could truthfully declare either, and the generated migration notes — which publish one section per verdict and invite a reader to count them — would present buckets whose completeness nothing established. A reader asking how many inputs the baseline could not process at all would be reading an answer that no rule made true.

`fails` names an outcome rather than a mechanism. A baseline that refuses its input with a diagnostic and exits nonzero, one that terminates on an unhandled exception, and one that writes part of an output tree and then gives up all alike completed no run, and all are `fails`. Separating them in the verdict was considered and rejected: the only mechanical signal available is the exit code, which encodes abnormal termination differently on each platform, and a verdict whose meaning depended on the runner would describe the machine rather than the tool. Where the distinction matters it belongs in the prose this section already requires, which records the observation in full.

Sampling refutes; it does not confirm. One contradicting sample proves `agrees`, `differs` or `fails` false, and no number of consistent samples proves any of them true. The zero-exit clause of `differs` is the one asymmetry: it is existential, so a single sample can satisfy it while only the exhausted sample set can refute it. 2.4.0 was measured reproducing one case's expected result on a single run in forty while aborting on the other thirty-nine, and a budget large enough to be confident of catching a branch that rare is a budget whose own outcome is decided by chance. The harness therefore does not attempt to re-derive `nondeterministic`: it is the verdict a case is forced into when one of the other three is refuted, it carries its measurement in prose, and no set of samples refutes it. It is consequently the one verdict a contributor may assert without the harness's agreement, and the generated migration notes list every case that asserts it, so the concession is published rather than silent.
