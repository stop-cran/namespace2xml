...
Consecutive comments are associated with the next entry, with one exception: comments preceding the first entry of a source are document-leading, as Section 20 classifies the first position for every format. Trailing comments with no following entry remain document-trailing comments.
...
**Open question (non-normative).** This clause gives a content-token value its role but does not relate the values one contribution assigns to those another assigned the same element. Converting an element-only contribution to mixed content has an answer, because Section 17.4 keeps mixed-content child elements from deep-merging across contributions and the values allocated for the converted content are therefore fresh. An occurrence promoted into a sequence by a repetition in another contribution is not covered: it keeps the value its own document's counter gave it, so its canonical address follows from the paragraph below while its place in the merged serialized stream does not follow from this one. A revision should settle that placement; until one does it is subject only to the Section 1 determinism requirement, and `KNOWN-LIMITS.md` records the reduction this leaves.
...
The specification text and the registry together form one versioned contract bundle. The bundle carries a revision identifier that changes whenever either artifact changes. `--version` reports that identifier as its `contract-bundle` field, so that a consumer, including an automated one, can determine exactly which contract a given binary implements and can cite it when reporting a defect.
