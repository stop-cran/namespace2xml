...
{
    private NamespaceRecord(NamespaceRecordKind kind, int line)
    {
        Kind = kind;
        Line = line;
    }
    ...
    internal static NamespaceRecord Ignored(int line) =>
        new(NamespaceRecordKind.Ignored, line);

    internal static NamespaceRecord Malformed(int line) =>
        new(NamespaceRecordKind.Malformed, line);

    internal static NamespaceRecord OfComment(string comment, int line) =>
        new(NamespaceRecordKind.Comment, line) { Comment = comment };

    internal static NamespaceRecord OfMask(string pattern, int line) =>
        new(NamespaceRecordKind.Mask, line) { Pattern = pattern };

    internal static NamespaceRecord OfEntry(string name, string value, int line) =>
        new(NamespaceRecordKind.Entry, line) { Name = name, Value = value };
}
